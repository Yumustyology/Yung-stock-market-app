...
export default function SearchCommand({
  ...
  // Listen for global watchlist changes (in case other components update it)
  useEffect(() => {
    const handler = (e: Event) => {
      try {
        // @ts-ignore - CustomEvent typing in DOM
        const detail = (e as CustomEvent)?.detail;
        if (!detail?.symbol) return;
        const { symbol, isInWatchlist } = detail;   // get the updated isInwWatchlist
        // When you pass a function to a state setter,
        //  React calls that function and gives you the previous state value as the first argument,
        //  which is commonly named prev.
        // prev is the previos value of the stocks and we transform it to
        // by default prev:[stocks] are set to empty array
        setStocks((prev = []) =>          
          prev.map((s) => (s.symbol === symbol ? { ...s, isInWatchlist } : s))
        );
      } catch (err) {
        // ignore
      }
    ...
    return () => window.removeEventListener('watchlist:changed', handler as EventListener);
  }, []);
